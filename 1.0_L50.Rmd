...
f <- paste0('out/l50_',Sys.Date(),'.csv')
write.csv(this.l50,file=f)
...
g <- ggplot(this.l50,aes(x=cohort,y=l50))+
  ...
  geom_point()+
  geom_text(aes(label=tot,y=Inf),vjust=3,size=1.4)+
  geom_text(aes(label=immat,y=Inf),vjust=6,size=1.4)+
  geom_text(aes(label='  n (total)',y=Inf,x=-Inf),vjust=3,size=1.4,hjust=0)+
  geom_text(aes(label='  n (immature)',y=Inf,x=-Inf),vjust=6,size=1.4,hjust=0)+
  ...
  scale_x_continuous(expand=c(0,0),limits=c(1970,max(this.l50$cohort)+1))+ 
  labs(y="L50 (mm)",x='Cohort')
g

f <- paste0('img/l50_',Sys.Date(),'.png')
ggsave(f,g,'png',width = 18,height = 8,units = 'cm')
```
