...
if (new) {
  ...
  ccox = d %>%  filter(cohort=="1994")  
       

  # fit annual model
  l50 <- ddply(d,  .(cohort),.fun= function(ccox){
 
    co <- unique(ccox$cohort)
     cat(paste(co, sep="..."))
    tab <- c(mat = nrow(ccox[ccox$mat == 1, ]), immat = nrow(ccox[ccox$mat == 0, ]), tot = nrow(ccox))
    mod <- try(glm(mat ~ length, data = ccox, family = binomial(link = logit)))
      if ("try-error" %in% class(mod)) {
         cat(paste0("no model for",co, "..."))
      ret <- c(l50 = NA, low = NA, high = NA, "(Intercept)" = NA, length = NA)
    } 
    if (!"try-error" %in% class(mod)){
      
     set.seed(27)
boots <- bootstraps(ccox, times = 999, apparent = TRUE)

fit_glm_on_bootstrap <- function(split) {
    glm(mat ~ length, family = binomial(link = logit), analysis(split))
}
boot_models <-
  boots %>% 
  dplyr::mutate(model = map(splits, fit_glm_on_bootstrap),
         coef_info = map(model, tidy))
boot_coefs <- 
  boot_models %>% 
  unnest(coef_info) %>% dplyr::select(id,term, estimate) %>%  tidyr::pivot_wider(values_from=estimate, names_from = term) %>%  dplyr::select(-id)

bootl <- apply(boot_coefs, 1, get.prop)
        bootq <- quantile(bootl, c(0.025, 0.975))
...
f <- paste0("img/l50_", year_of_assess, ".png")
...
f <- paste0("img/l50_", year_of_assess, "_FR.png")
